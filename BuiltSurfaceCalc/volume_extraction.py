...
def process_single_year(year, base_raster_dir, shapefile_path):
    ...
    # 构建年份对应的栅格文件路径
    raster_path = os.path.join(base_raster_dir, f"GHS_BUILT_V_E{year}_GLOBE_R2023A_4326_30ss_V1_0/GHS_BUILT_V_E{year}_GLOBE_R2023A_4326_30ss_V1_0.tif")
    ...
    for i, stat in enumerate(stats):
        ...
        if volume is None:
            volumes.append(np.nan)
            print(f"警告: {country_name} 体积值为None")
        elif math.isnan(volume):
            volumes.append(np.nan)
            print(f"警告: {country_name} 体积值为NaN")
        else:
            ...
            try:
                ...
                if valid_count <= 5:  # 只打印前5个有效结果作为示例
                    print(f"✓ {country_name}: {volume:,.0f}")
            except (OverflowError, ValueError) as e:
                volumes.append(np.nan)
                print(f"警告: {country_name} 体积值转换失败: {volume} - {e}")
    
    # 6. 添加结果到原始数据
    countries["total_vol"] = volumes
    
    # 7. 输出结果
    output_dir = "vol_results"
    os.makedirs(output_dir, exist_ok=True)
    
    # 保存为GeoPackage格式（避免Shapefile限制）
    # output_gpkg = os.path.join(output_dir, "countries_with_volume.gpkg")
    # countries.to_file(output_gpkg, driver='GPKG', encoding='utf-8')
    # print(f"GeoPackage结果已保存至: {output_gpkg}")
    
    # 保存CSV属性表 - 按年份命名
    output_csv = os.path.join(output_dir, f"volume_statistics_{year}.csv")
    # 选择所有非几何列
    ...
    if valid_count > 0:
        # 过滤掉NaN值
        valid_volumes = countries["total_vol"].dropna()
        
        print(f"\n{year}年体积统计摘要:")
        print(f"有效国家数量: {valid_count}/{len(countries)}")
        print(f"总和: {valid_volumes.sum():,.0f}")
        print(f"平均值: {valid_volumes.mean():,.0f}")
        print(f"最大值: {valid_volumes.max():,.0f}")
        print(f"最小值: {valid_volumes.min():,.0f}")
        
        # 找到最大值和最小值对应的国家
        max_idx = valid_volumes.idxmax()
        min_idx = valid_volumes.idxmin()
        
        print(f"建筑体积最大的国家: {countries.loc[max_idx, 'NAM_0']} ({valid_volumes.max():,.0f})")
        print(f"建筑体积最小的国家: {countries.loc[min_idx, 'NAM_0']} ({valid_volumes.min():,.0f})")
    else:
        print(f"警告: {year}年没有有效的体积计算结果")
        print("可能的原因:")
...
def main():
    ...
    """
    print("开始批量处理1975-2030年建筑体积数据...")
    start_time = time.time()
    
    # 配置路径
    base_raster_dir = "D:/data/Built_Volume/"  # 栅格数据根目录
    ...
    if failed_years:
        ...
        for year in failed_years:
            expected_path = os.path.join(base_raster_dir, f"GHS_BUILT_V_E{year}_GLOBE_R2023A_4326_30ss_V1_0/GHS_BUILT_V_E{year}_GLOBE_R2023A_4326_30ss_V1_0.tif")
